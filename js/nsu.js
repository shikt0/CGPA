...
/* ── NSU GRADING SCALE (4.0) ── */
function getGradeInfo(marks) {
  const m = parseFloat(marks);
  if (isNaN(m)) return { letter: '', point: null, fail: false };
  if (m >= 93) return { letter: 'A',  point: 4.00, fail: false };
  if (m >= 90) return { letter: 'A−', point: 3.70, fail: false };
  if (m >= 87) return { letter: 'B+', point: 3.30, fail: false };
  if (m >= 83) return { letter: 'B',  point: 3.00, fail: false };
  if (m >= 80) return { letter: 'B−', point: 2.70, fail: false };
  if (m >= 77) return { letter: 'C+', point: 2.30, fail: false };
  if (m >= 73) return { letter: 'C',  point: 2.00, fail: false };
  if (m >= 70) return { letter: 'C−', point: 1.70, fail: false };
  if (m >= 67) return { letter: 'D+', point: 1.30, fail: false };
  if (m >= 60) return { letter: 'D',  point: 1.00, fail: false };
...
function calculateCGPA() {
  ...
  const gradeLabel = c >= 3.70 ? '🏆 Outstanding' : c >= 3.30 ? '🌟 Excellent' : c >= 3.00 ? '✅ Very Good' : c >= 2.50 ? '📘 Good' : c >= 2.00 ? '📗 Satisfactory' : '⚠️ Needs Improvement';
...
function saveSemester() {
  ...
  localStorage.setItem('nsu_courses', JSON.stringify(data));
  showToast('Semester saved successfully!');
}

function loadSemester() {
  const raw = localStorage.getItem('nsu_courses');
