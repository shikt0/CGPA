...
function getGradeInfo(marks) {
  ...
  if (m >= 85)  return { letter: 'A',  point: 3.75, fail: false };
  if (m >= 80)  return { letter: 'B+', point: 3.50, fail: false };
  if (m >= 75)  return { letter: 'B',  point: 3.25, fail: false };
  if (m >= 70)  return { letter: 'C+', point: 3.00, fail: false };
  if (m >= 65)  return { letter: 'C',  point: 2.75, fail: false };
  if (m >= 60)  return { letter: 'D+', point: 2.50, fail: false };
  if (m >= 50)  return { letter: 'D',  point: 2.25, fail: false };
...
function calculateCGPA() {
  ...
  else if (c >= 2.25) gradeLabel = '📗 Satisfactory';
...
function saveSemester() {
  ...
  localStorage.setItem('aiub_courses', JSON.stringify(data)); /* fixed: was nu_courses */
  showToast('Semester saved successfully!');
}

function loadSemester() {
  const raw = localStorage.getItem('aiub_courses'); /* fixed: was nu_courses */
...
/* ── FAQ TOGGLE ── (was missing — this is why FAQ didn't work) */
